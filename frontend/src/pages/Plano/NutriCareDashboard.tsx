...
import FoodPrescriptionModal, {
  Food,
} from "../../components/Modais/FoodPrescriptionModal/FoodPrescriptionModal";
...
const NutriCareDashboard = () => {
  ...
  useEffect(() => {
    ...
    const totalMacroCalories =
      totals.protein * 4 + totals.carbohydrate * 4 + totals.fat * 9;
    if (totalMacroCalories > 0) {
      setNutritionData([
        {
          name: "Carboidratos",
          value: parseFloat(
            (((totals.carbohydrate * 4) / totalMacroCalories) * 100).toFixed(1)
          ),
          color: "#4CAF50",
        },
        {
          name: "Proteínas",
          value: parseFloat(
            (((totals.protein * 4) / totalMacroCalories) * 100).toFixed(1)
          ),
          color: "#1687C6",
        },
        {
          name: "Gorduras",
          value: parseFloat(
            (((totals.fat * 9) / totalMacroCalories) * 100).toFixed(1)
          ),
          color: "#FFC107",
        },
      ]);
  ...
  const nutritionTableData = useMemo(() => {
    ...
    return [
      {
        nutrient: "Calorias totais",
        value: `${totals.calories.toFixed(0)}kcal`,
      },
      { nutrient: "Proteínas totais", value: `${totals.protein.toFixed(1)}g` },
      { nutrient: "Lipídios totais", value: `${totals.fat.toFixed(1)}g` },
      {
        nutrient: "Carboidratos totais",
        value: `${totals.carbohydrate.toFixed(1)}g`,
      },
                ...
                <div key={meal.id} className={styles.mealRow}>
                  <div className={styles.mealControls}>
                    ...
                    <select
                      value={meal.food}
                      onChange={(e) =>
                        updateMeal(meal.id, "food", e.target.value as string)
                      }
                        ...
                        meal.foods.map((food) => (
                          <div key={food.id} className={styles.foodItem}>
                            <div className={styles.foodInfo}>
                              ...
                              <p>
                                {food.quantity} &times;{" "}
                                {food.chosen_serving.serving_description}
                              </p>
                            </div>
                            <div className={styles.foodNutrition}>
                              <span>
                                {food.calculated_nutrition.calories.toFixed(0)}{" "}
                                kcal
                              </span>
                            </div>
                            <button
